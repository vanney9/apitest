(function (global, factory) {
    ...
    var gradients = {
        linearGradient: function linearGradient(node, gradNode, opacity) {
            ...
            gradNode.each(function (child) {
                col = d3.color(child.attrs.get('stop-color'));
    ...
    function touch(node, v) {
        ...
        root._scheduled = d3.timeout(redraw(root));
    }
    ...
    CanvasElement.prototype = {
        ...
        setAttribute: function setAttribute(attr, value) {
            if (attr === 'class') {
                ...
                if (!this.attrs) this.attrs = d3.map();
    ...
    var originalAttr = d3.selection.prototype.attr;
    var defaultFactor;

    d3.selection.prototype.attr = selectionAttr;
    d3.selection.prototype.canvas = asCanvas;
    d3.selection.prototype.canvasResolution = canvasResolution;
    ...
    function selectCanvas(context, factor) {
        ...
        if (typeof context === 'string') {
            context = d3.select(context).node();
            if (!context) return s;
        }
